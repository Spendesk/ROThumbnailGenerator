...
open class ROThumbnail {
  ...
  fileprivate var supportedFiletypes: [String: ROThumbnailGenerator] = [:]
  ...
  fileprivate func initializeSupportedGenerators() {
    ...
    // If you want to add your own thumbnail generator
    // use the method ROThumbnail.sharedInstance.addThumbnailGenerator(yourGenerator)
  ...
  open func getThumbnail(_ url: URL) -> UIImage? {
    ...
    guard let appropriateThumbnailGenerator = supportedFiletypes[fileExtension.lowercased()],
      let thumbnail = appropriateThumbnailGenerator.getThumbnail(url) else {
        return nil
    }

    // Don't perform compression if image quality is set to 100%
    guard imageQuality < 1 else {
      return thumbnail
    }

    // Image quality of the thumbnail is defined in the imageQuality variable, can be setted from outside
    guard let jpeg = thumbnail.jpegData(compressionQuality: imageQuality),
      let compressedThumbnail = UIImage(data: jpeg) else {
        return nil
    }
